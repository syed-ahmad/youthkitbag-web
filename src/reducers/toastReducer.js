...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case DELETE_KITBAG_TRADE:
      return { currentMessage: action.payload.message, currentStyle: 'warning', hasShown: false };
    case API_KITBAG_ERROR:
      return { currentMessage: action.payload.data.message, currentStyle: 'error', errors: action.payload.data.errors, hasShown: true };
    case LOGIN_FAILURE: 
      return { currentMessage: action.payload.data.message, currentStyle: 'error', hasShown: true };
    case SHOWN_TOAST:
